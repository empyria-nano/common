 ...
 * carrying a pre-rendered string.
 * @param {Object} schema - JSON schema to validate against.
 * @param {Object} [options] - Forwarded to `ata-validator`'s `Validator` constructor
 *   (see {@link createValidator} for the common flags).
 * @returns {(data: *) => { valid: boolean, errors: Array<Object> }} Function that returns the
 *   verdict and, when invalid, `ata-validator`'s raw error objects (empty array when valid).
 *   Never throws on a validation failure.
 */
export function createChecker(schema, options) {
	...
	return (data) => {
		const result = v.validate(data)
		return { valid: result.valid, errors: result.valid ? [] : result.errors }
	}
}
 ...
 * @param {Object} schema - JSON schema to validate against.
 * @param {*} input - Value to validate.
 * @param {Object} [options] - Forwarded to {@link createChecker} (see its docs).
 * @returns {{ valid: boolean, errors: Array<Object> }} The verdict, plus `ata-validator`'s raw
 *   error objects when invalid (empty array when valid).
 */
