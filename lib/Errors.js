...
import { templating } from '@principia/classification'

/**
 * Principia's base error type. Carries a machine-readable `errorName`/`errorCode`
 ...
 */
let PrincipiaError = function (message, errorName, errorCode) {
	this.message = message
	this.errorName = errorName
	this.errorCode = errorCode
	Error.captureStackTrace(this, PrincipiaError)
}
inherits(PrincipiaError, Error)
 ...
 *   `templating` in `@principia/classification`).
 * @returns {(opts?: Object) => PrincipiaError} Function that, given template values, returns a
 *   new {@link PrincipiaError} with its `message` filled in and `params` set to the values used.
 */
let ErrorCreator = function (options = {}) {
	...
	let fnc = function (opts = {}) {
		...
		let resultError = new PrincipiaError(message, options.errorName || errorCode, errorCode)
...
/**
 * Principia's catalog of pre-built error factories, one per well-known failure case.
 ...
 * {@link PrincipiaError} instance ready to `throw`.
 */
let BaseErrors = {
	...
	NotInited: ErrorCreator({
		errorCode: 599997,
		errorName: 'NotInited',
		message: 'Principia is not inited',
	}),
...
export { PrincipiaError, ErrorCreator, BaseErrors }
