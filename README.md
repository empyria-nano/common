...
| [lib/Ata.js](./lib/Ata.js)             | JSON schema building blocks (`string`, `number`, `integer`, `bool`, `enumType`, `constant`, `nullable`, `array`, `discriminated`, `email`, `isoDateTime`, ...) and validation — throwing (`defineSchema`, `createValidator`, `validate`), transform-intent aliases (`parse`, `createParser`), or non-throwing/structured (`createChecker`, `checkSchema`, now returning the parsed `data`) — built on [ata-validator](https://www.npmjs.com/package/ata-validator). See _Schema builders_ and _Validation options_ below. |
...
Each returns a plain JSON Schema fragment; compose them in `defineSchema(map, opts)`.

| Builder                                                                                 | Produces                                                                                    |
| --------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| `bool(default?)`                                                                        | `{ type: 'boolean' }`                                                                       |
| `string(default?, minLength \| { minLength, maxLength, pattern, format, description })` | `{ type: 'string', … }`                                                                     |
| `number(default?, { min, max, exclusiveMin, exclusiveMax, multipleOf, description })`   | `{ type: 'number', … }`                                                                     |
| `integer(default?, …)`                                                                  | `{ type: 'integer', … }` — use for ports/counts; rejects `3.5`                              |
| `enumType(list, default?, { type, description })`                                       | `{ enum, type? }`                                                                           |
| `constant(value, { description })`                                                      | `{ const: value }`                                                                          |
| `nullable(schema)`                                                                      | `type: [t, 'null']`, or `{ anyOf: [schema, { type: 'null' }] }`                             |
| `array(items?, { minItems, maxItems, uniqueItems, default, description })`              | `{ type: 'array', … }`                                                                      |
| `email() / password() / ip4() / ip6()`                                                  | `{ type: 'string', format: 'empyria-*' }` (see `EMPYRIA_FORMATS`)                           |
| `isoDateTime() / isoDate() / isoTime() / duration() / uri() / uuid()`                   | `{ type: 'string', format: … }` (ata built-in formats)                                      |
| `logLevel() / nodeEnv() / tmpFile() / logFile()`                                        | domain presets                                                                              |
| `discriminated(prop, { value: schema, … }, { description })`                            | JSON Schema **v1** `propertyDependencies` — an object whose shape depends on `prop`'s value |

### Dialect

`defineSchema(map, { dialect: 'v1' \| '2020-12' \| 'draft-07' \| <uri> })` stamps `$schema`
(`ata-validator` reads it to pick the dialect). `setDefaultDialect('v1')` / `getDefaultDialect()`
set a process-wide default a per-call `dialect` overrides. v1 differs from 2020-12 only in
`$dynamicRef` scoping and unlocks `propertyDependencies`; `discriminated()` works on any
dialect (ata supports the keyword as an extension) but `dialect: 'v1'` makes it spec-legitimate.

`defineSchema(map, { strict: 'unevaluated' })` sets `unevaluatedProperties: false` while
leaving `additionalProperties` open — the composable form of strict, for schemas meant to be
`allOf`-combined.
